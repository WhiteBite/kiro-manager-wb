...
## 🔧 Диагностика и оптимизация Kiro

### Автоматическая диагностика лагов

Если Kiro тормозит, используй встроенные инструменты диагностики:

```bash
# Полная диагностика (проверка GPU, памяти, настроек)
...
**Что проверяется:**
- ❌ **SwiftShader** — software rendering вместо GPU (главная причина лагов!)
- 🧠 **Память** — утечки в Extension Host
- ⚙️ **Настройки** — hardware acceleration, telemetry
- 📁 **File watchers** — node_modules в watcher'ах

### Проверка GPU

```bash
# Проверить используется ли hardware acceleration
python autoreg/scripts/check_kiro_gpu.py
```

Если видишь **SwiftShader** — это проблема! Решения:
1. Обнови драйвера видеокарты
2. Добавь в `argv.json`: `"ignore-gpu-blocklist": true`
3. Проверь DirectX/OpenGL

### WebGL тест в браузере

Открой `tests/test-gpu.html` в браузере чтобы проверить GPU:
- ✅ **Hardware acceleration** — всё ок
- ❌ **SwiftShader** — нужно чинить
...
Standalone десктопное приложение без зависимости от VS Code/Kiro.

### Development

```bash
npm run electron:install  # Установить зависимости Electron (первый раз)
...
```bash
npm run electron:build:win  # Собрать Windows installer
```
